...
import {Text, View, SafeAreaView, StyleSheet, AsyncStorage} from 'react-native';
...
function levelfrom(msg, rootinfo) {
  if (!rootinfo) {
    ...
    return parseInt(msg.substr(0, 1)) + 2;
  } else if (rootinfo.nodetype == 'ntgrupoestab') {
    return parseInt(msg.substr(0, 1)) + 3;
  }
}
            ...
            if (grupoestab.id == id && level == 3) {
              ...
              grupoestab.estabs.items.map(estab => {
                if (estab.id == id && level == 4) {
                  console.log(estab.id + ' - ' + estab.descr);
                  result = estab.descr;
                }
              });
              {
